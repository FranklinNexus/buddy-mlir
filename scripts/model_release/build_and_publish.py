...
def build_variant(
    ...
    native: bool = False,
    llvm_install_dir: Path | None = None,
    buddy_build_dir: Path | None = None,
) -> Path:
    if buddy_build_dir is not None:
        model_build = buddy_build_dir
    else:
    ...
    # Native builds (riscv64 runner) use a prebuilt LLVM from the GitHub
    # release; cross builds (x64 runner) use the in-tree LLVM + GNU toolchain.
    llvm_root = llvm_install_dir if native else paths["llvm_build"]
    cmd = [
        python,
    ...
    if not native:
        ...
        # Native riscv64: the DAP/DIP custom commands default BUDDY_OPT_ATTR to
        # empty, which emits soft-float objects that cannot link against the
        # lp64d C++ runtime. Force the RVV/double-float attributes.
        if platform.machine().startswith("riscv64"):
            cmd += [
                "--cmake-args=-DBUDDY_TARGET_TRIPLE=riscv64-unknown-linux-gnu",
                "--cmake-args=-DBUDDY_OPT_ATTR=+m,+d,+v",
            ]
        if buddy_build_dir is not None:
            cmd += ["--buddy-mlir-build-dir", buddy_build_dir]
...
def main() -> int:
    ...
    ap.add_argument(
        "--native",
        action="store_true",
        help="Build for the host architecture (use on a riscv64 runner); "
        "no RISC-V cross toolchain is required. Implies --skip-deps.",
    )
    ap.add_argument(
        "--llvm-install-dir",
        type=Path,
        default=None,
        help="Prebuilt LLVM/MLIR install tree (e.g. from a GitHub release); "
        "skips the LLVM/GNU-toolchain builds.",
    )
    ap.add_argument(
        "--buddy-build-dir",
        type=Path,
        default=None,
        help="Reuse an existing buddy-mlir build tree (e.g. the one built by "
        "the ci-common action) as the model build dir.",
    )
    ap.add_argument(
        "--deps-stamp",
        type=Path,
    ...
    llvm_install_dir = (
        args.llvm_install_dir.expanduser().resolve()
        if args.llvm_install_dir is not None
        else None
    )
    buddy_build_dir = (
        args.buddy_build_dir.expanduser().resolve()
        if args.buddy_build_dir is not None
        else None
    )

    if args.native and llvm_install_dir is None:
        print(
            "error: --native requires --llvm-install-dir"
            " (or run the full from-source cross build)",
            file=sys.stderr,
        )
        return 1

    skip_deps = args.skip_deps or args.native or llvm_install_dir is not None
    if not skip_deps:
        ...
        try:
            ...
            for spec_entry in target["variants"]:
                ...
                rax = build_variant(
                    ...
                    buddy_build_dir=buddy_build_dir,
                )
                meta = {
                    "family": family,
                    "variant": variant,
