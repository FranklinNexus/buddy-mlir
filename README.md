...
The target automatically uses the repository's configure helper for glibc's
sysdeps enumeration and sets `MAKEINFO=:` for the GNU toolchain build. The
helper avoids a host `ls` operand-limit issue, while skipping optional glibc
Info manuals keeps builds working from non-ASCII source paths; neither affects
the compiler, headers, or runtime libraries.
